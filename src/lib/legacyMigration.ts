...
type LegacyNote = { id: string; title?: string; body?: string; favorite?: boolean; collectionId?: string | null; updatedAt?: number };
...
async function deterministicId(userId: string, kind: string, legacyId: string) { return stableUuid(`${userId}:${kind}:${legacyId}`); }

async function ensureCourse(userId: string, legacy: LegacyCourse) {
  const id = await deterministicId(userId, "course", legacy.id);
  const { data: existing, error: lookupError } = await supabase.from("courses").select("id").eq("id", id).maybeSingle();
  if (lookupError) throw lookupError;
  if (existing) return id;
  const { error } = await supabase.from("courses").insert({ id, user_id: userId, name: legacy.name?.trim() || "Untitled course", description: legacy.description?.trim() ?? "", color: legacy.color ?? "sky", ...(legacy.createdAt ? { created_at: new Date(legacy.createdAt).toISOString() } : {}) });
  if (error) {
    const { data: raced, error: raceLookupError } = await supabase.from("courses").select("id").eq("id", id).maybeSingle();
...
async function ensureCollection(userId: string, courseId: string, legacy: LegacyCollection) {
  ...
  const { data: existing, error: lookupError } = await supabase.from("collections").select("id").eq("id", id).maybeSingle();
  if (lookupError) throw lookupError;
  if (existing) return id;
  const { error } = await supabase.from("collections").insert({ id, user_id: userId, course_id: courseId, name: legacy.name.trim() });
  if (error) {
    const { data: raced, error: raceLookupError } = await supabase.from("collections").select("id").eq("id", id).maybeSingle();
    if (raceLookupError) throw raceLookupError;
    if (!raced) throw error;
  }
  return id;
...
async function ensureNote(userId: string, courseId: string, collectionId: string | null, legacy: LegacyNote) {
  ...
  const { data: existing, error: lookupError } = await supabase.from("notes").select("id").eq("id", id).maybeSingle();
  if (lookupError) throw lookupError;
  if (existing) return;
  const { error } = await supabase.from("notes").insert({ id, user_id: userId, course_id: courseId, collection_id: collectionId, title: legacy.title ?? "Untitled note", body: legacy.body ?? "", favorite: Boolean(legacy.favorite), updated_at: new Date(legacy.updatedAt ?? Date.now()).toISOString(), source_id: sourceId });
  if (error) {
    const { data: raced, error: raceLookupError } = await supabase.from("notes").select("id").eq("id", id).maybeSingle();
    if (raceLookupError) throw raceLookupError;
    if (!raced) throw error;
  }
}
...
export async function runLegacyMigrationOnce(userId: string): Promise<boolean> {
  ...
  try {
    ...
    for (const legacyCourse of courses) {
      ...
      const legacyCollections = read<LegacyCollection[]>(`${scoped(COLLECTIONS_KEY, userId)}:${legacyCourse.id}`, []);
      ...
      const legacyNotes = read<LegacyNote[]>(`${scoped(NOTES_KEY, userId)}:${legacyCourse.id}`, []);
      ...
      for (const legacyNote of notes) {
        ...
        const collectionId = legacyNote.collectionId ? collectionIdMap.get(legacyNote.collectionId) ?? null : null;
    ...
    if (courses.length === 0) {
      const { count, error: countError } = await supabase.from("courses").select("id", { count: "exact", head: true });
      ...
      if (!count && (globalNotes.length || globalCollections.length)) {
        const courseId = await ensureCourse(userId, { id: "legacy-global-notes", name: "My Notes", description: "Imported from your previous notes.", color: "sky" });
        const collectionIdMap = new Map<string, string>();
        for (const legacyCollection of globalCollections) {
          if (!legacyCollection || typeof legacyCollection.id !== "string" || typeof legacyCollection.name !== "string") continue;
          collectionIdMap.set(legacyCollection.id, await ensureCollection(userId, courseId, legacyCollection));
        }
        for (const legacyNote of globalNotes) {
          if (!legacyNote || typeof legacyNote.id !== "string") continue;
          await ensureNote(userId, courseId, legacyNote.collectionId ? collectionIdMap.get(legacyNote.collectionId) ?? null : null, legacyNote);
        }
        changed = true;
      }
    }

    const engine = read<Record<string, number> | null>(scoped(ENGINE_KEY, userId), read<Record<string, number> | null>(ENGINE_KEY, null));
    const theme = localStorage.getItem(scoped(THEME_KEY, userId)) ?? localStorage.getItem(THEME_KEY);
    const displayName = localStorage.getItem(scoped(DISPLAY_NAME_KEY, userId)) ?? localStorage.getItem(DISPLAY_NAME_KEY);

    const { data: profile, error: profileReadError } = await supabase.from("profiles").select("id,theme,display_name,liquid_density,liquid_transparency,liquid_clearness,liquid_gel,liquid_bounce_stiffness,liquid_bounce_damping").eq("id", userId).maybeSingle();
    if (profileReadError) throw profileReadError;

    const patch: Record<string, unknown> = {};
    const cloudLiquidPristine = !profile || [
      [profile.liquid_density, 12],
      [profile.liquid_transparency, 45],
      [profile.liquid_clearness, 35],
      [profile.liquid_gel, 55],
      [profile.liquid_bounce_stiffness, 260],
      [profile.liquid_bounce_damping, 18],
    ].every(([actual, fallback]) => actual == null || Number(actual) === fallback);

    // Legacy liquid settings are imported only into a brand-new/default cloud profile.
