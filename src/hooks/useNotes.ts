...
type NoteRow = { id: string; title: string | null; body: string | null; favorite: boolean | null; collection_id: string | null; source_id: string | null; created_at: string; updated_at: string };
type CollectionRow = { id: string; name: string };
type NotesScope = "editor" | "study-hub";
type NoteWritePayload = { op: "insert" | "update" | "delete"; id: string; ids?: string[]; values?: Record<string, unknown>; rollback?: () => void };
const EMPTY_NOTES: Note[] = [];
const EMPTY_COLLECTIONS: Collection[] = [];

function toNote(r: NoteRow): Note { const c = new Date(r.created_at).getTime(), u = new Date(r.updated_at).getTime(); return { id: r.id, title: r.title ?? "Untitled note", body: r.body ?? "", favorite: Boolean(r.favorite), collectionId: r.collection_id, sourceId: r.source_id, createdAt: Number.isFinite(c) ? c : u, updatedAt: Number.isFinite(u) ? u : c }; }
function dedupeNotes(a: Note[]) { const m = new Map<string, Note>(); for (const n of a) { const e = m.get(n.id); if (!e || n.updatedAt >= e.updatedAt) m.set(n.id, n); } return Array.from(m.values()); }
...
export function useNotes(courseId?: string, userId?: string, scope: NotesScope = "editor") {
  ...
  const notesQuery = useQuery({ queryKey: notesKey, enabled, queryFn: async () => { const { data, error } = await supabase.from("notes").select("id,title,body,favorite,collection_id,source_id,created_at,updated_at").eq("course_id", courseId!).order("created_at", { ascending: false }); if (error) throw error; return dedupeNotes(((data ?? []) as NoteRow[]).map(toNote)); } });
  ...
  const writeNote = useMutation({ mutationFn: async (p: NoteWritePayload) => { if (p.op === "insert") { const { error } = await supabase.from("notes").insert({ id: p.id, user_id: userId!, course_id: courseId!, ...p.values }); if (error) throw error; return; } if (p.op === "update") { const { error } = await supabase.from("notes").update({ ...p.values, updated_at: new Date().toISOString() }).eq("id", p.id).eq("course_id", courseId!); if (error) throw error; return; } const ids = p.ids?.length ? p.ids : [p.id]; const { error } = await supabase.from("notes").delete().in("id", ids).eq("course_id", courseId!); if (error) throw error; }, onError: (_e, p) => { p.rollback?.(); invalidateNotes(); }, onSuccess: invalidateNotes });
  ...
  const createNote = useCallback((initial?: { sourceId?: string | null; title?: string; body?: string; favorite?: boolean; collectionId?: string | null }) => {
    if (!enabled) return "";
    ...
    const current = dedupeNotes(qc.getQueryData<Note[]>(notesKey) ?? notes);

    // Only explicit restore/import operations deduplicate by source identity.
    // Independently-created notes, including blank notes, always receive a new ID.
    if (sourceId) {
      const existingBySource = current.find(n => n.sourceId === sourceId || n.id === sourceId);
      if (existingBySource) { setSelectedId(existingBySource.id); return existingBySource.id; }
    }

    const now = Date.now();
    const note: Note = { id: newId(), title, body, favorite, collectionId, sourceId, createdAt: now, updatedAt: now };
    const previous = current;
    patchNotesCache(p => [note, ...p]);
    setSelectedId(note.id);
    writeNote.mutate({ op: "insert", id: note.id, values: { title: note.title, body: note.body, favorite: note.favorite, collection_id: note.collectionId, source_id: note.sourceId }, rollback: () => qc.setQueryData<Note[]>(notesKey, previous) });
    return note.id;
  }, [enabled, filter, notes, notesKey, patchNotesCache, qc, writeNote]);

  const updateNote = useCallback((id: string, p: Partial<Omit<Note, "id">>) => { const previous = dedupeNotes(qc.getQueryData<Note[]>(notesKey) ?? notes); patchNotesCache(a => a.map(n => n.id === id ? { ...n, ...p, updatedAt: Date.now() } : n)); const v: Record<string, unknown> = {}; if (p.title !== undefined) v.title = p.title; if (p.body !== undefined) v.body = p.body; if (p.favorite !== undefined) v.favorite = p.favorite; if (p.collectionId !== undefined) v.collection_id = p.collectionId; if (p.sourceId !== undefined) v.source_id = p.sourceId; if (Object.keys(v).length) writeNote.mutate({ op: "update", id, values: v, rollback: () => qc.setQueryData<Note[]>(notesKey, previous) }); }, [notes, notesKey, patchNotesCache, qc, writeNote]);
  const deleteNote = useCallback((id: string) => { const current = dedupeNotes(qc.getQueryData<Note[]>(notesKey) ?? notes), target = current.find(n => n.id === id); if (!target) return; const previous = current; patchNotesCache(a => a.filter(n => n.id !== id)); setSelectedId(cur => cur === id ? null : cur); writeNote.mutate({ op: "delete", id, rollback: () => qc.setQueryData<Note[]>(notesKey, previous) }); }, [notes, notesKey, patchNotesCache, qc, writeNote]);
  const toggleFavorite = useCallback((id: string) => { const current = dedupeNotes(qc.getQueryData<Note[]>(notesKey) ?? notes), n = current.find(x => x.id === id); if (!n) return; const previous = current, next = !n.favorite; patchNotesCache(a => a.map(x => x.id === id ? { ...x, favorite: next, updatedAt: Date.now() } : x)); writeNote.mutate({ op: "update", id, values: { favorite: next }, rollback: () => qc.setQueryData<Note[]>(notesKey, previous) }); }, [notes, notesKey, patchNotesCache, qc, writeNote]);
