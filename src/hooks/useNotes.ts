...
type QueueState={tail:Promise<void>;nextRevision:number};
const noteWriteQueues=new Map<string,QueueState>();
function queueNoteWrite<T>(key:string,op:NoteWritePayload["op"],requestedRevision:number,task:(expectedRevision:number)=>Promise<T>):Promise<T>{
  const existing=noteWriteQueues.get(key);
  const state=existing??{tail:Promise.resolve(),nextRevision:requestedRevision};
  if(op==="update"){
    if(!existing)state.nextRevision=requestedRevision;
    const expectedRevision=state.nextRevision;
...
export function useNotes(courseId?:string,userId?:string,scope:NotesScope="editor"){
  ...
  const writeNote=useMutation({mutationFn:async(p:NoteWritePayload)=>queueNoteWrite(`${userId}:${courseId}:${p.id}`,p.op,p.expectedRevision??0,async(expectedRevision)=>{if(p.op==="insert"){const{error}=await supabase.from("notes").insert({id:p.id,user_id:userId!,course_id:courseId!,...p.values,revision:0});if(error)throw error;return;}if(p.op==="update"){const nextRevision=expectedRevision+1;const{data,error}=await supabase.from("notes").update({...p.values,revision:nextRevision,updated_at:new Date().toISOString()}).eq("id",p.id).eq("course_id",courseId!).eq("revision",expectedRevision).select("id").maybeSingle();if(error)throw error;if(!data)throw new Error("Note changed elsewhere. Your editor was refreshed with the newest saved version.");return;}const{data,error}=await supabase.from("notes").delete().eq("id",p.id).eq("course_id",courseId!).eq("revision",expectedRevision).select("id").maybeSingle();if(error)throw error;if(!data)throw new Error("Note changed elsewhere. Delete was cancelled to protect the newer version.");}),onError:(_e,p)=>{p.rollback?.();invalidateNotes();},onSuccess:invalidateNotes});
  const writeCollection=useMutation({mutationFn:async(p:{op:"insert"|"update"|"delete";id:string;values?:Record<string,unknown>})=>{if(p.op==="insert"){const{error}=await supabase.from("collections").insert({id:p.id,user_id:userId!,course_id:courseId!,...p.values}as never);if(error)throw error;return;}if(p.op==="update"){const{error}=await supabase.from("collections").update({...p.values,updated_at:new Date().toISOString()}).eq("id",p.id).eq("course_id",courseId!);if(error)throw error;return;}const{error}=await supabase.from("collections").delete().eq("id",p.id).eq("course_id",courseId!);if(error)throw error;}},onSettled:()=>{invalidateCollections();invalidateNotes();}});
  ...
  const deleteNote=useCallback((id:string)=>{const current=dedupeNotes(qc.getQueryData<Note[]>(notesKey)??notes),target=current.find(n=>n.id===id);if(!target)return Promise.resolve(false);const previous=current;patchNotesCache(a=>a.filter(n=>n.id!==id));setSelectedId(cur=>cur===id?null:cur);return writeNote.mutateAsync({op:"delete",id,expectedRevision:target.revision,rollback:()=>qc.setQueryData<Note[]>(notesKey,previous)}).then(()=>true).catch(()=>false);},[notes,notesKey,patchNotesCache,qc,writeNote]);
