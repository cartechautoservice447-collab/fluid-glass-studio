  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[85vh] overflow-y-auto border-white/25 bg-white/10 backdrop-blur-2xl sm:max-w-lg" style={{ backdropFilter: "blur(calc(var(--liquid-density) + 8px)) saturate(170%)" }}>
      ...
      <section className="space-y-6 pt-2">
        ...
        <LiquidSlider label="Liquid Saturation" hint="Controls the color intensity of the liquid glass surfaces." value={liquid.saturation} min={50} max={250} display={`${liquid.saturation}%`} onChange={(saturation) => setLiquid({ saturation })} />
        <LiquidSlider label="Liquid Sheen" hint="Controls the strength of the soft reflective highlight across glass surfaces." value={liquid.sheen} min={0} max={100} display={`${liquid.sheen}%`} onChange={(sheen) => setLiquid({ sheen })} />
        <LiquidSlider label="Liquid Refraction" hint="Controls the intensity of refractive distortion around liquid glass edges." value={liquid.refraction} min={0} max={100} display={`${liquid.refraction}%`} onChange={(refraction) => setLiquid({ refraction })} />
