  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger === "icon" ? (
          <Button type="button" size="icon" variant="outline" className="shrink-0 border-white/30 bg-white/10 backdrop-blur-xl hover:bg-white/20" aria-label="Open engine customization">
            <Settings2 className="size-4" />
          </Button>
        ) : (
          <Button variant="outline" className="border-white/30 bg-white/10 backdrop-blur-xl hover:bg-white/20">
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto border-white/25 bg-white/10 backdrop-blur-2xl sm:max-w-lg" style={{ backdropFilter: "blur(calc(var(--liquid-density) + 8px)) saturate(170%)" }}>
        <DialogHeader>
          ...
          <DialogDescription>Tune the Apple Liquid Glass Engine. Every change is live and remembered.</DialogDescription>
        </DialogHeader>
        ...
        <section className="space-y-6 pt-2">
          ...
          <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/20 bg-white/5 p-4">
            <div><p className="text-[0.68rem] font-bold uppercase tracking-[0.22em] text-foreground">Appearance</p><p className="mt-1 text-xs text-muted-foreground">{theme === "dark" ? "Night mode — obsidian liquid" : "Day mode — bright liquid"}</p></div>
            <ThemeToggle />
          </div>

          <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/20 bg-white/5 p-4">
            <div><p className="text-[0.68rem] font-bold uppercase tracking-[0.22em] text-foreground">Pure Black</p><p className="mt-1 text-xs text-muted-foreground">Flat black background instead of the gradient glow. Glass panels stay as they are.</p></div>
          ...
          <div className="space-y-4 rounded-2xl border border-white/20 bg-white/5 p-4">
            <LiquidSlider label="Liquid Bounce · Stiffness" hint="Spring stiffness driving the gel bounce on hover, click and drag." value={liquid.bounceStiffness} min={100} max={500} step={5} display={`${liquid.bounceStiffness}`} onChange={(bounceStiffness) => setLiquid({ bounceStiffness })} />
            <LiquidSlider label="Liquid Bounce · Damping" hint="Lower damping = wobblier liquid; higher damping settles instantly." value={liquid.bounceDamping} min={10} max={40} display={`${liquid.bounceDamping}`} onChange={(bounceDamping) => setLiquid({ bounceDamping })} />
          </div>
