...
export function StudySessionRing({ segment, remaining }: Props) {
  ...
  return (
    <div className="relative mx-auto mt-3 size-44" aria-label={`${segment?.label ?? "Session"} timer ${formatTime(remaining)} remaining`}>
      <svg viewBox={`0 0 ${size} ${size}`} className="size-full -rotate-90" role="img" aria-hidden="true">
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="currentColor" strokeWidth="8" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashOffset} className={segment?.kind === "rest" ? "text-white/70 transition-[stroke-dashoffset] duration-300" : "text-foreground transition-[stroke-dashoffset] duration-300"} />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">{segment?.label ?? "Complete"}</span>
