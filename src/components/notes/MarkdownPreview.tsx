...
import { Check, Copy, Maximize2, PanelRight, PanelTop } from "lucide-react";
...
export function MarkdownPreview({ body }: { body: string }) {
  ...
  if (!body.trim()) {
    return (
      <div className="flex min-h-full items-center justify-center rounded-2xl border border-white/10 bg-white/[0.025] px-6 py-16 text-center">
        <div>
          <p className="text-base font-medium text-white/75">Nothing to preview yet</p>
          <p className="mt-1 text-sm text-white/40">Start writing in Edit mode and your finished note will appear here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-full flex-col rounded-2xl border border-white/10 bg-white/[0.025] shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
      <div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-2 border-b border-white/10 bg-[#0d1117]/90 px-3 py-2 backdrop-blur-xl">
        <div className="flex items-center gap-1.5 text-[0.65rem] font-medium uppercase tracking-[0.16em] text-white/40">
        ...
        <div className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.035] p-0.5">
          <button type="button" onClick={() => setWidth("comfortable")} className={`flex h-7 items-center gap-1.5 rounded-md px-2 text-[0.68rem] transition ${width === "comfortable" ? "bg-white/10 text-white" : "text-white/45 hover:text-white/75"}`} title="Comfortable reading width">
            <PanelTop className="size-3.5" /> Read
          </button>
          <button type="button" onClick={() => setWidth("wide")} className={`flex h-7 items-center gap-1.5 rounded-md px-2 text-[0.68rem] transition ${width === "wide" ? "bg-white/10 text-white" : "text-white/45 hover:text-white/75"}`} title="Wide reading width">
            <PanelRight className="size-3.5" /> Wide
          </button>
          <button type="button" onClick={copyMarkdown} className="flex h-7 items-center gap-1.5 rounded-md px-2 text-[0.68rem] text-white/55 transition hover:bg-white/10 hover:text-white" title="Copy Markdown">
            {copied ? <Check className="size-3.5" /> : <Copy className="size-3.5" />} {copied ? "Copied" : "Copy"}
          </button>
          <button type="button" onClick={() => void copyText(body)} className="hidden h-7 items-center gap-1.5 rounded-md px-2 text-[0.68rem] text-white/45 transition hover:bg-white/10 hover:text-white sm:flex" title="Open browser reading mode">
        ...
        <ReactMarkdown
          ...
          components={{
            ...
            code: ({ className, children, ...rest }: ComponentPropsWithoutRef<"code">) => {
              ...
              return (
                <div className="group my-6 overflow-hidden rounded-xl border border-white/10 bg-black/25 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                  <div className="flex items-center justify-between border-b border-white/10 px-3 py-2 text-[0.62rem] uppercase tracking-[0.14em] text-white/35">
                    <span>{match[1]}</span>
                    <button type="button" onClick={() => void copyText(text)} className="rounded-md px-2 py-1 normal-case tracking-normal text-white/45 opacity-70 transition hover:bg-white/10 hover:text-white group-hover:opacity-100" title="Copy code"><Copy className="mr-1 inline size-3" />Copy</button>
                  </div>
                  <SyntaxHighlighter language={match[1]} style={glassCodeTheme} PreTag="div">{text}</SyntaxHighlighter>
                </div>
              );
            },
          }}
