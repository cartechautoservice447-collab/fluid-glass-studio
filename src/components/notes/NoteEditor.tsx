        ...
        <div className="flex min-h-10 flex-wrap items-center gap-2 border-b border-white/10 p-4 pr-12">
          <Select value={note.id} onValueChange={onSelectNote}>
            <SelectTrigger
              ...
              aria-label="Select note topic"
              title="Select note topic"
            >
          ...
          <Select
            ...
            onValueChange={(value) => onUpdate(note.id, { collectionId: value === NO_COLLECTION ? null : value })}
          >
          ...
          <div className="relative w-full max-w-sm overflow-hidden rounded-[30px] border border-white/25 bg-white/[0.12] p-6 text-foreground shadow-[inset_0_1px_2px_rgba(255,255,255,.45),0_20px_60px_rgba(0,0,0,.35)] backdrop-blur-2xl">
            ...
            <div className="relative">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Are you sure you want to delete <span className="font-medium text-foreground">{note.title || "this note"}</span>? This action cannot be undone.
              </p>
              <div className="mt-6 flex justify-end gap-2">
                <button
                  type="button"
                  onClick={() => setConfirmDelete(false)}
                  className="rounded-2xl border border-white/15 bg-white/[0.08] px-4 py-2.5 text-sm font-medium transition hover:bg-white/[0.14]"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="rounded-2xl border border-white/15 bg-white/[0.14] px-4 py-2.5 text-sm font-semibold transition hover:bg-white/[0.2]"
                >
                  Delete
                </button>
              </div>
