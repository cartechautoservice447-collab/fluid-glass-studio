        ...
        if (segment.type === "output") {
          return (
            <section key={`output-${index}`} className="my-3 overflow-hidden rounded-xl border border-[#30363d] bg-[#010409] shadow-[0_8px_24px_rgba(0,0,0,0.22)]" aria-label="Terminal output">
              ...
              <textarea value={segment.content} onFocus={(event) => { onActiveIndexChange(index); onActiveTextarea(event.currentTarget); }} onClick={(event) => { onActiveIndexChange(index); onActiveTextarea(event.currentTarget); }} onChange={(event) => { resizeTextarea(event.currentTarget); onChangeSegment(index, event.target.value); }} onBlur={(event) => fullResizeTextarea(event.currentTarget)} ref={(element) => { refs.current[index] = element; resizeTextarea(element); }} aria-label="Code output" spellCheck={false} className={`block min-h-[4rem] w-full resize-none overflow-hidden bg-transparent px-4 py-3 font-mono text-[#c9d1d9] outline-none ${readability === "great" ? "text-[0.9rem] leading-7" : readability === "best" ? "text-[0.86rem] leading-7" : "text-[0.82rem] leading-6"}`} />
            </section>
          );
        }

        return (
          <textarea key={`text-${index}`} ref={(element) => { refs.current[index] = element; resizeTextarea(element); }} value={segment.content} onFocus={(event) => { onActiveIndexChange(index); onActiveTextarea(event.currentTarget); }} onClick={(event) => { onActiveIndexChange(index); onActiveTextarea(event.currentTarget); }} onChange={(event) => { resizeTextarea(event.currentTarget); onChangeSegment(index, event.target.value); }} onBlur={(event) => fullResizeTextarea(event.currentTarget)} aria-label="Note body" placeholder={index === 0 ? "Write markdown here…" : undefined} spellCheck={false} className={`block min-h-[8rem] w-full resize-none overflow-hidden bg-transparent p-0 text-[#c9d1d9] outline-none placeholder:text-[#8b949e] ${rhythm.textSize} ${rhythm.lineHeight}`} style={{ fontFamily: "'Fira Code', 'JetBrains Mono', 'Consolas', monospace" }} />
        );
      })}
    </div>
  );
