...
import { Bell, BellRing, Check, Clock3, X } from "lucide-react";
...
export function GlassReminderCenter() {
  ...
  const addReminder = () => {
    ...
    if (!title.trim() || !Number.isFinite(timestamp) || timestamp <= Date.now()) return;
    const next = [{ id: crypto.randomUUID(), title: title.trim(), when: timestamp, createdAt: Date.now() }, ...load()];
    save(next); setReminders(next); setTitle(""); setWhen("");
  };
  ...
  return <>
    <div className="fixed bottom-5 right-5 z-[80]">
      <Button type="button" size="icon" onClick={() => setOpen((value) => !value)} className="size-12 rounded-full border border-white/20 bg-white/10 text-foreground shadow-xl backdrop-blur-xl hover:bg-white/15" aria-label="Study reminders">
        {upcoming.length ? <BellRing className="size-5" /> : <Bell className="size-5" />}
      </Button>
    </div>

    {open && <div className="fixed bottom-20 right-5 z-[81] w-[min(360px,calc(100vw-40px))] overflow-hidden rounded-[28px] border border-white/20 bg-black/40 text-foreground shadow-2xl backdrop-blur-2xl">
