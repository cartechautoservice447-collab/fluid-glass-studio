    ...
    <>
      ...
      >
        <div className="flex h-full flex-col justify-between">
          <div className="flex items-start justify-between gap-3">
            <span className="flex size-11 items-center justify-center rounded-2xl border border-white/10" style={{ backgroundColor: `color-mix(in oklab, ${accent} 55%, transparent)` }}>
          ...
          <div className="flex items-center border-t border-white/10 pt-3">
            <span className="text-[0.68rem] text-muted-foreground">{lastEditedAt ? `Last edited ${relativeDate(lastEditedAt)}` : "No notes yet"}</span>
          </div>
        </div>

        <button type="button" onClick={confirmDelete} className="absolute bottom-3 right-3 z-10 flex size-7 items-center justify-center text-foreground/65 transition-colors hover:text-foreground focus-visible:outline-none" aria-label={`Delete ${course.name}`} title="Delete course">
      ...
      {confirmOpen && (
        <div
          ...
          data-state="open"
          className="fixed inset-0 z-40 flex items-center justify-center bg-black/35 p-5 backdrop-blur-md"
          role="alertdialog"
          aria-modal="true"
        ...
        >
          <GlassPanel
            ...
            className="relative z-50 w-full max-w-sm p-6 text-foreground"
            glassRadius={30}
            glassBezel={40}
          >
            ...
            <div className="relative">
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">Are you sure you want to delete <span className="font-medium text-foreground">{course.name}</span>? This will permanently delete the course and its notes.</p>
              <div className="mt-6 flex justify-end gap-2">
                <button type="button" onClick={cancelDelete} className="rounded-2xl border border-white/15 bg-white/[0.08] px-4 py-2.5 text-sm font-medium transition hover:bg-white/[0.14]">Cancel</button>
