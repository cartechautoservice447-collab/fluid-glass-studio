import { BookOpen, CalendarDays, Clock3, ShieldCheck, Timer, X } from "lucide-react";
...
function DistractionMode({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setRemaining(left);
      if (left === 0) save({ ...session, active: false, endsAt: null });
    };
    tick();
  ...
  const requestStart = () => setConfirmOpen(true);
    ...
    return (
      <div className="fixed inset-0 z-[200] flex min-h-screen items-center justify-center overflow-hidden bg-black/70 p-5 backdrop-blur-2xl" role="dialog" aria-modal="true" aria-label="Distraction mode active">
        ...
        <div className="relative w-full max-w-xl rounded-[34px] border border-white/20 bg-black/40 p-7 text-center shadow-2xl backdrop-blur-2xl sm:p-10">
          <div className="mx-auto flex size-16 items-center justify-center rounded-2xl border border-white/15 bg-white/[0.07] shadow-inner"><ShieldCheck className="size-7" /></div>
          ...
          <div className="mt-7 grid grid-cols-1 gap-2 text-left sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-3"><div className="flex items-center gap-2 text-[11px] text-muted-foreground"><BookOpen className="size-3.5" /> Course</div><p className="mt-1 truncate text-sm font-semibold">{session.courseName}</p></div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-3"><div className="flex items-center gap-2 text-[11px] text-muted-foreground"><CalendarDays className="size-3.5" /> Date</div><p className="mt-1 text-sm font-semibold">{dateLabel}</p></div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-3"><div className="flex items-center gap-2 text-[11px] text-muted-foreground"><Clock3 className="size-3.5" /> Planned</div><p className="mt-1 text-sm font-semibold">{session.durationHours} hour{session.durationHours === 1 ? "" : "s"}</p></div>
          </div>
          ...
          <div className="mt-7 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 text-xs leading-5 text-muted-foreground">Notes, course folders, and dashboard controls are unavailable during this session.</div>
        </div>
      </div>
    );
  }
    ...
    <>
      <div className="fixed inset-0 z-[190] flex items-center justify-center bg-black/35 p-5 backdrop-blur-md" role="dialog" aria-modal="true" aria-label="Distraction Mode setup">
        <div className="w-full max-w-md rounded-[30px] border border-white/20 bg-black/35 p-6 shadow-2xl backdrop-blur-2xl sm:p-7">
          <div className="flex items-center justify-between"><div><p className="text-lg font-semibold">Distraction Mode</p><p className="mt-1 text-xs text-muted-foreground">Create a focused study session.</p></div><button type="button" onClick={() => onOpenChange(false)} className="rounded-full p-2 text-muted-foreground hover:bg-white/10" aria-label="Close"><X className="size-4" /></button></div>
          <div className="mt-6 space-y-4">
            <label className="block"><span className="mb-2 block text-xs font-medium text-muted-foreground">Course name</span><input value={session.courseName} onChange={(event) => setSession((current) => ({ ...current, courseName: event.target.value }))} placeholder="e.g. Accountancy" className="w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm outline-none transition focus:border-white/25" /></label>
            <div className="grid grid-cols-2 gap-3"><label className="block"><span className="mb-2 block text-xs font-medium text-muted-foreground">Study hours</span><input type="number" min={1} max={24} value={session.durationHours} onChange={(event) => setSession((current) => ({ ...current, durationHours: Math.min(24, Math.max(1, Number(event.target.value) || 1)) }))} className="w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm outline-none" /></label><label className="block"><span className="mb-2 block text-xs font-medium text-muted-foreground">Date</span><input type="date" value={session.date} onChange={(event) => setSession((current) => ({ ...current, date: event.target.value }))} className="w-full rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm outline-none" /></label></div>
          </div>
          <button type="button" onClick={requestStart} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-foreground px-5 py-3 text-sm font-semibold text-background transition hover:opacity-90"><ShieldCheck className="size-4" /> Start Distraction Mode</button>
          <p className="mt-3 text-center text-[11px] leading-5 text-muted-foreground">After confirmation, the website remains locked until the selected study time ends.</p>
        </div>
      </div>

      {confirmOpen && (
        <div className="fixed inset-0 z-[220] flex items-center justify-center bg-black/55 p-5 backdrop-blur-md" role="alertdialog" aria-modal="true" aria-labelledby="distraction-confirm-title">
          <div className="w-full max-w-sm rounded-[28px] border border-white/20 bg-black/45 p-6 text-center shadow-2xl backdrop-blur-2xl">
            <div className="mx-auto flex size-14 items-center justify-center rounded-2xl border border-white/15 bg-white/[0.07]"><ShieldCheck className="size-6" /></div>
            <h3 id="distraction-confirm-title" className="mt-5 text-xl font-semibold">Start Distraction Mode?</h3>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Once confirmed, you cannot turn Distraction Mode off. The website will stay locked until your selected study time finishes.</p>
            <div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setConfirmOpen(false)} className="rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-medium hover:bg-white/10">Cancel</button><button type="button" onClick={confirmStart} className="rounded-2xl bg-foreground px-4 py-3 text-sm font-semibold text-background hover:opacity-90">Confirm & Lock</button></div>
          </div>
        </div>
      )}
    </>
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-6 overflow-y-auto px-2 py-2">
      <div className="liquid-panel flex flex-wrap items-center justify-between gap-4 rounded-3xl p-5" style={{ backgroundColor: "var(--water-gel-bg)", backdropFilter: "blur(var(--liquid-density, 12px)) saturate(200%) contrast(105%)", borderTop: "1px solid rgba(255, 255, 255, 0.4)", boxShadow: "inset 0 1px 2px 0 rgba(255, 255, 255, 0.5), inset 0 -2px 4px 0 rgba(0, 0, 0, 0.25), 0 8px 32px 0 rgba(0, 0, 0, 0.37)" }}>
        <div className="flex items-center gap-4"><span aria-hidden className="flex size-14 shrink-0 items-center justify-center rounded-full" style={{ background: "radial-gradient(60% 60% at 35% 30%, oklch(0.85 0.15 250 / 0.9), oklch(0.55 0.2 285 / 0.6) 55%, transparent 75%)", boxShadow: "0 0 24px 4px oklch(0.6 0.2 270 / 0.45)" }}><BookOpen className="size-6 text-white drop-shadow" /></span><div><h1 className="text-xl font-semibold tracking-tight text-foreground">Welcome back{displayName ? `, ${displayName}` : ""}! 👋</h1><p className="text-sm text-muted-foreground">Select a course folder to access your workspace</p></div></div>
        <div className="flex items-center gap-2"><ThemeToggle /><EngineSettingsModal /></div>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <button type="button" onClick={() => setDistractionOpen(true)} className="liquid-panel group flex min-h-28 items-center gap-4 rounded-3xl p-5 text-left transition duration-200 hover:-translate-y-0.5 hover:brightness-110" style={{ backgroundColor: "var(--water-gel-bg)", backdropFilter: "blur(var(--liquid-density, 12px)) saturate(180%)", borderTop: "1px solid rgba(255,255,255,.32)" }}><span className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-foreground shadow-inner"><ShieldCheck className="size-5" /></span><span><span className="block text-base font-semibold text-foreground">Distraction Mode</span><span className="mt-1 block text-xs text-muted-foreground">Lock the workspace and study without distractions</span></span></button>
