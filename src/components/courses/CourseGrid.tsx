import { Bell, History, Timer, Zap } from "lucide-react";
...
export function CourseGrid({ courses, noteCounts, lastEdited, hiddenCourseId, onOpenCourse, onCreateCourse }: Props) {
  ...
  return <div className="flex h-full min-h-0 w-full flex-col gap-6 overflow-y-auto px-2 py-2">
    <div className="flex flex-wrap items-center justify-between gap-3 px-1 pt-1">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2"><span className="inline-flex items-center gap-1 rounded-xl border border-white/10 bg-white/[.06] px-3 py-2 text-xs text-muted-foreground"><Zap className="size-3.5"/>Performance: <b className="text-foreground">{performance === "ultra" ? "Ultra" : "High"}</b></span><ThemeToggle /><EngineSettingsModal /></div>
    </div>
