...
export function StudyFeaturesPanel({ courseName, notes, selectedId, onSelectNote, onCreateNote, onUpdateNote, onDeleteNote, onToggleFavorite, onRestoreNote }: Props) {
  ...
  const achievements = [uniqueNotes.length >= 1 && "First note", uniqueNotes.length >= 10 && "10 notes", pom.sessions >= 5 && "5 focus sessions", pom.minutes >= 300 && "5 focus hours"].filter(Boolean) as string[];
  ...
  const moveToTrash = (note: Note) => {
    ...
    if (selectedId === note.id) {
      const nextVisible = uniqueNotes.find((item) => item.id !== note.id);
      if (nextVisible) onSelectNote(nextVisible.id);
    }
  };
